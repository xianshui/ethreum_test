...
class S3ItemExporter(CompositeItemExporter):
    ...
    def upload(self, location, filename, type, block_date, item_id):
            table = pcsv.read_csv(filename)
            new_filename = filename.replace('csv', 'parquet')
            parquet.write_table(table, new_filename)
            path = "{}/{}/date={}/{}".format(self.CHAIN, new_filename.split('.')[0], block_date, item_id)
            print(f'>>>>>new file name:{new_filename}, {block_date} {item_id} path:{path}')

            res = self.s3.upload_file(new_filename, location, path)
